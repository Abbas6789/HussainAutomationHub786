"""
app.py  — PUBLIC USER PORTAL
Hussain Automation Hub · Run: streamlit run app.py
All files are in the same root directory — no subfolders needed.
"""
import os, json, tempfile, time, hashlib
from pathlib import Path
from datetime import datetime

import streamlit as st

st.set_page_config(
    page_title="Hussain Automation Hub",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded",
)

# ── Root-level imports (no shared/ prefix) ────────────────────────────────────
from styles         import inject_global_css, whatsapp_button, brand_header
from firebase_utils import (
    get_db, get_user, upsert_user,
    get_balance, get_system_message, get_platform_toggles, get_settings,
)
from ai_meta   import generate_metadata_ai
from uploaders import upload_youtube, upload_facebook, upload_tiktok, process_video_ffmpeg

inject_global_css()

GOOGLE_CLIENT_ID = "249087096220-dkg4b73rq7s5sqji0fq8p6n42h1lvvq4.apps.googleusercontent.com"
SALT = "hussain_hub_salt_786"

# ── Session defaults ──────────────────────────────────────────────────────────
for k, v in {"user": None, "upload_logs": [], "meta": {}, "nav": "🏠 Dashboard"}.items():
    if k not in st.session_state:
        st.session_state[k] = v

db = get_db()

# ════════════════════════════════════════════════════════════════════════════════
#  SIDEBAR
# ════════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    brand_header("PUBLIC PORTAL")
    st.markdown("---")

    if st.session_state.user:
        u = st.session_state.user
        if u.get("picture"):
            st.markdown(
                f'<img src="{u["picture"]}" style="width:54px;height:54px;border-radius:50%;'
                f'border:2px solid #6c63ff;margin-bottom:8px;">',
                unsafe_allow_html=True
            )
        st.markdown(f"**{u['name']}**")
        st.markdown(f"<span style='color:var(--muted);font-size:0.75rem;'>{u['email']}</span>",
                    unsafe_allow_html=True)
        st.markdown("---")

        live_bal = get_balance(db, u["email"]) if db else u.get("balance", 0.0)
        st.metric("💰 Account Balance", f"PKR {live_bal:,.2f}")
        st.markdown("---")

        st.session_state.nav = st.radio(
            "nav", ["🏠 Dashboard", "📤 Upload & Post", "📊 History", "👤 Profile"],
            label_visibility="collapsed"
        )
        st.markdown("---")
        whatsapp_button("💬 Support")
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🚪 Logout", use_container_width=True):
            st.session_state.user = None
            st.session_state.nav  = "🏠 Dashboard"
            st.rerun()
    else:
        st.markdown("""
        <p style='font-size:0.78rem;color:var(--muted);line-height:1.7;'>
        Welcome to Hussain Automation Hub.<br>Log in to access the tools.
        </p>""", unsafe_allow_html=True)
        st.markdown("---")
        whatsapp_button("💬 Support / Contact Admin")


# ════════════════════════════════════════════════════════════════════════════════
#  HOME / LOGIN
# ════════════════════════════════════════════════════════════════════════════════
def page_home():
    _, col, _ = st.columns([1, 2, 1])
    with col:
        brand_header("SOCIAL MEDIA AUTOMATION PLATFORM")
        st.markdown("---")

        sys_msg = get_system_message(db) if db else "Welcome! Log in to automate your social media."
        st.info(f"📢 {sys_msg}")
        st.markdown("<br>", unsafe_allow_html=True)

        st.markdown("""
        <div class='hub-card' style='text-align:center;'>
          <div style='font-size:2.5rem;margin-bottom:0.5rem;'>⚡</div>
          <h2 style='font-family:Syne,sans-serif;font-size:1.6rem;margin:0 0 0.5rem;'>
            One Upload. Every Platform.
          </h2>
          <p style='color:var(--muted);font-size:0.85rem;'>
            Post to YouTube Shorts, Facebook Reels & TikTok simultaneously
            with AI-generated titles and hashtags.
          </p>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)

        # ── Google One Tap
        st.markdown("<p style='text-align:center;color:var(--muted);font-size:0.8rem;'>Sign in with Google</p>",
                    unsafe_allow_html=True)
        st.components.v1.html(f"""
        <script src="https://accounts.google.com/gsi/client" async defer></script>
        <div id="g_id_onload"
             data-client_id="{GOOGLE_CLIENT_ID}"
             data-context="signin"
             data-ux_mode="popup"
             data-callback="handleCredentialResponse"
             data-auto_prompt="false">
        </div>
        <div class="g_id_signin"
             data-type="standard" data-shape="rectangular"
             data-theme="filled_black" data-text="sign_in_with"
             data-size="large" data-logo_alignment="left"
             style="display:flex;justify-content:center;margin:1rem 0;">
        </div>
        <script>
        function handleCredentialResponse(r) {{
            const p = JSON.parse(atob(r.credential.split('.')[1]));
            window.parent.postMessage({{type:'google_login', payload:p}}, '*');
        }}
        </script>
        """, height=90)

        st.markdown("<br>", unsafe_allow_html=True)

        # ── Email / password fallback
        with st.expander("🔐 Log in with Email & Password"):
            email_in = st.text_input("Email", placeholder="you@email.com", key="le")
            pass_in  = st.text_input("Password", type="password", key="lp")
            if st.button("Log In", use_container_width=True):
                if email_in and pass_in:
                    db_user = get_user(db, email_in) if db else None
                    if db_user:
                        attempt = hashlib.sha256((SALT + pass_in).encode()).hexdigest()
                        if db_user.get("password_hash","") == attempt:
                            if not db_user.get("approved", False):
                                st.warning("⏳ Account pending admin approval.")
                                whatsapp_button("Contact Admin")
                            else:
                                st.session_state.user = {
                                    "email":   email_in,
                                    "name":    db_user.get("name", email_in),
                                    "picture": db_user.get("picture", ""),
                                    "balance": db_user.get("balance", 0.0),
                                }
                                st.rerun()
                        else:
                            st.error("Invalid credentials.")
                    else:
                        st.error("No account found. Contact admin." if db else
                                 "⚠️ Firebase not connected.")
                else:
                    st.warning("Enter email and password.")

        st.markdown("---")
        st.markdown("<div style='text-align:center;'>", unsafe_allow_html=True)
        whatsapp_button("💬 Need help? Contact Admin")
        st.markdown("</div>", unsafe_allow_html=True)


# ════════════════════════════════════════════════════════════════════════════════
#  DASHBOARD
# ════════════════════════════════════════════════════════════════════════════════
def page_dashboard():
    u       = st.session_state.user
    toggles = get_platform_toggles(db) if db else {"youtube":True,"facebook":True,"tiktok":True}

    hc1, hc2, hc3 = st.columns([3, 1, 1])
    with hc1:
        brand_header(f"WELCOME BACK, {u['name'].upper()}")
    with hc2:
        if u.get("picture"):
            st.markdown(
                f'<img src="{u["picture"]}" style="width:48px;height:48px;border-radius:50%;'
                f'border:2px solid #6c63ff;margin-top:1.5rem;">',
                unsafe_allow_html=True
            )
    with hc3:
        st.markdown("<div style='margin-top:1.5rem;'>", unsafe_allow_html=True)
        whatsapp_button("💬 Support")
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("---")

    live_bal = get_balance(db, u["email"]) if db else 0.0
    m1,m2,m3,m4 = st.columns(4)
    m1.metric("💰 Balance",          f"PKR {live_bal:,.2f}")
    m2.metric("📤 Posts This Session", len([l for l in st.session_state.upload_logs if l.get("ok")]))
    m3.metric("📡 Active Platforms",  sum(v for v in toggles.values()))
    m4.metric("🏷️ AI Hashtags",       "Auto")
    st.markdown("---")

    pc1, pc2, pc3 = st.columns(3)
    for col, name, key, icon in [
        (pc1,"YouTube","youtube","▶️"),
        (pc2,"Facebook","facebook","📘"),
        (pc3,"TikTok","tiktok","🎵"),
    ]:
        on    = toggles.get(key, True)
        badge = "badge-green" if on else "badge-red"
        label = "ACTIVE" if on else "DISABLED BY ADMIN"
        col.markdown(f"""
        <div class='hub-card' style='text-align:center;'>
          <div style='font-size:2rem;'>{icon}</div>
          <div style='font-weight:700;margin:4px 0;'>{name}</div>
          <span class='{badge}'>{label}</span>
        </div>""", unsafe_allow_html=True)


# ════════════════════════════════════════════════════════════════════════════════
#  UPLOAD & POST
# ════════════════════════════════════════════════════════════════════════════════
def page_upload():
    st.markdown("<h2 style='font-family:Syne,sans-serif;'>📤 Upload & Post Everywhere</h2>",
                unsafe_allow_html=True)

    settings      = get_settings(db) if db else {}
    toggles       = get_platform_toggles(db) if db else {"youtube":True,"facebook":True,"tiktok":True}
    anthropic_key = settings.get("anthropic_api_key", os.getenv("ANTHROPIC_API_KEY",""))

    uploaded = st.file_uploader("Drop your video (MP4, MOV, AVI)", type=["mp4","mov","avi","mkv"])

    if not uploaded:
        st.markdown("""
        <div class='hub-card' style='text-align:center;padding:3rem;'>
          <div style='font-size:3rem;'>🎬</div>
          <p style='color:var(--muted);'>Upload a video above — AI will instantly generate
          your title, description & hashtags.</p>
        </div>""", unsafe_allow_html=True)
        return

    left, right = st.columns([3, 2], gap="large")

    with left:
        st.video(uploaded)
        if not st.session_state.meta or st.session_state.meta.get("_file") != uploaded.name:
            with st.spinner("🤖 AI generating metadata…"):
                meta = generate_metadata_ai(uploaded.name, anthropic_key)
                meta["_file"] = uploaded.name
                st.session_state.meta = meta
            st.success("✅ AI metadata generated!")

        meta  = st.session_state.meta
        title = st.text_input("Title (AI-generated, editable)", value=meta.get("title",""))
        desc  = st.text_area("Description", value=meta.get("description",""), height=90)
        tags  = meta.get("hashtags", [])
        st.text_area("Hashtags", value=" ".join(tags), height=70)
        if st.button("🔄 Regenerate Metadata"):
            st.session_state.meta = {}
            st.rerun()

    with right:
        st.markdown("<div class='hub-card'><h4 style='font-family:Syne,sans-serif;margin-top:0;'>🎯 Platforms</h4>",
                    unsafe_allow_html=True)
        post_yt = st.checkbox("▶️ YouTube Shorts",  value=toggles.get("youtube",True),  disabled=not toggles.get("youtube",True))
        post_fb = st.checkbox("📘 Facebook Reels",  value=toggles.get("facebook",True), disabled=not toggles.get("facebook",True))
        post_tt = st.checkbox("🎵 TikTok",          value=toggles.get("tiktok",True),   disabled=not toggles.get("tiktok",True))
        st.markdown("</div>", unsafe_allow_html=True)

        yt_key = settings.get("youtube_api_key",  os.getenv("YOUTUBE_API_KEY",""))
        fb_tok = settings.get("facebook_token",   os.getenv("FACEBOOK_ACCESS_TOKEN",""))
        tt_key = settings.get("tiktok_api_key",   os.getenv("TIKTOK_API_KEY",""))

        active = []
        if post_yt and toggles.get("youtube"):  active.append("YouTube")
        if post_fb and toggles.get("facebook"): active.append("Facebook")
        if post_tt and toggles.get("tiktok"):   active.append("TikTok")

        st.markdown("<br>", unsafe_allow_html=True)

        if not active:
            st.warning("Select at least one platform.")
        else:
            st.markdown(f"""
            <div style='background:rgba(108,99,255,0.1);border:1px solid rgba(108,99,255,0.3);
                        border-radius:8px;padding:0.8rem 1rem;margin-bottom:1rem;font-size:0.82rem;'>
              Posting to: <b>{', '.join(active)}</b>
            </div>""", unsafe_allow_html=True)

            if st.button("🚀  POST EVERYWHERE", use_container_width=True):
                logs = []
                with tempfile.TemporaryDirectory() as tmp:
                    tmp_p = Path(tmp)
                    src   = tmp_p / uploaded.name
                    src.write_bytes(uploaded.getvalue())
                    prog = st.progress(0)
                    stat = st.empty()

                    for i, plat in enumerate(active):
                        stat.markdown(f"**{plat}** — converting…")
                        proc = process_video_ffmpeg(src, tmp_p, plat)
                        stat.markdown(f"**{plat}** — uploading…")

                        if plat == "YouTube":
                            res = upload_youtube(proc, title, desc, tags, yt_key)
                        elif plat == "Facebook":
                            res = upload_facebook(proc, title, desc, tags, fb_tok)
                        else:
                            res = upload_tiktok(proc, title, desc, tags, tt_key)

                        res["ts"] = datetime.now().strftime("%H:%M:%S")
                        logs.append(res)
                        prog.progress((i+1)/len(active))

                stat.empty(); prog.empty()
                for log in logs:
                    if log["ok"]:
                        st.success(f"✅ **{log['platform']}** posted at {log['ts']}")
                    else:
                        st.error(f"❌ **{log['platform']}** — {log.get('error')}")
                st.session_state.upload_logs.extend(logs)


# ════════════════════════════════════════════════════════════════════════════════
#  HISTORY
# ════════════════════════════════════════════════════════════════════════════════
def page_history():
    st.markdown("<h2 style='font-family:Syne,sans-serif;'>📊 Upload History</h2>",
                unsafe_allow_html=True)
    if not st.session_state.upload_logs:
        st.info("No posts yet.")
        return
    for log in reversed(st.session_state.upload_logs):
        color = "#43e97b" if log["ok"] else "#ff6584"
        icon  = "✅" if log["ok"] else "❌"
        st.markdown(f"""
        <div class='hub-card' style='display:flex;justify-content:space-between;align-items:center;'>
          <span>{icon} <b style='color:{color};'>{log.get("platform","?")}</b></span>
          <span style='color:var(--muted);font-size:0.8rem;'>{log.get("ts","")}</span>
          <span style='color:var(--muted);font-size:0.78rem;'>
            {log.get("error","") if not log["ok"] else log.get("video_id",log.get("publish_id","ok"))}
          </span>
        </div>""", unsafe_allow_html=True)
    if st.button("🗑️ Clear History"):
        st.session_state.upload_logs = []
        st.rerun()


# ════════════════════════════════════════════════════════════════════════════════
#  PROFILE
# ════════════════════════════════════════════════════════════════════════════════
def page_profile():
    u = st.session_state.user
    st.markdown("<h2 style='font-family:Syne,sans-serif;'>👤 My Profile</h2>",
                unsafe_allow_html=True)
    c1, c2 = st.columns([1, 3])
    with c1:
        if u.get("picture"):
            st.markdown(
                f'<img src="{u["picture"]}" style="width:110px;height:110px;border-radius:50%;'
                f'border:3px solid #6c63ff;">',
                unsafe_allow_html=True
            )
    with c2:
        st.markdown(f"### {u['name']}")
        st.markdown(f"📧 `{u['email']}`")
        live_bal = get_balance(db, u["email"]) if db else 0.0
        st.metric("💰 Live Balance", f"PKR {live_bal:,.2f}")


# ════════════════════════════════════════════════════════════════════════════════
#  ROUTER
# ════════════════════════════════════════════════════════════════════════════════
if not st.session_state.user:
    page_home()
else:
    nav = st.session_state.nav
    if "Upload"  in nav: page_upload()
    elif "History" in nav: page_history()
    elif "Profile" in nav: page_profile()
    else:                  page_dashboard()

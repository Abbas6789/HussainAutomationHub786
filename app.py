...
All API credentials fetched securely from Firebase — zero hardcoded keys.
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS notice (id INTEGER PRIMARY KEY, message TEXT DEFAULT '')''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS videos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER, title TEXT, hashtags TEXT,
        filename TEXT, uploaded_at TEXT, status TEXT DEFAULT 'queued'
    )''')
...
def get_cached_remote():
    """Cache remote config for 60 seconds to avoid hammering Firebase."""
...
def get_whatsapp_number():
    ...
    return remote.get("support_whatsapp", "923461785207")

def get_premium_price():
    remote = get_cached_remote()
    return remote.get("premium_price", "$19")

def get_free_limit():
    remote = get_cached_remote()
    return int(remote.get("free_upload_limit", 3))
...
def render_auth_page():
    ...
    with col2:
        if pic.exists():
            ...
            with c2: st.image(str(pic), width=90)
        st.markdown(f"""
...
def render_dashboard(user: dict):
    ...
    # Notice board (Firebase → SQLite fallback)
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "🔗 Social Connect", "📹 Video Automation",
    ...
    with tab1:
        ...
        # Fetch live credentials from Firebase (no keys in source)
        social_creds = fb.get_social_credentials()
        google_creds = fb.get_google_auth_config()
        ...
        for col, platform, icon, has_key, description in [
            ...
            with col:
                ...
                if st.button(f"Connect {platform}", key=f"connect_{platform}", use_container_width=True):
                    if has_key:
                        st.success(f"✅ {platform} credentials loaded from Firebase! Full OAuth flow ready for implementation.")
                    else:
                        st.warning(f"⚠️ Admin hasn't configured {platform} API keys yet. Contact admin.")
    ...
    with tab2:
        ...
        if user.get('plan') == 'free':
            st.warning(f"⚠️ Free plan: {free_limit} uploads/month. Upgrade to Premium for unlimited uploads.")
        ...
        with col1:
            ...
            st.multiselect("Target Platforms", ["YouTube","Facebook","TikTok"], default=["YouTube"])
            if st.button("⚡ Generate & Queue", key="gen_video", use_container_width=True):
                if uploaded_file:
                    # Check free plan limit
                    if user.get('plan') == 'free' and len(get_user_videos(user['id'])) >= free_limit:
                        st.error(f"❌ Free plan limit ({free_limit}/month) reached. Upgrade to Premium!")
                    else:
                        titles = {
                        ...
                        title = custom_title if custom_title else titles.get(niche,"My Video")
                        hashtags = tags.get(niche,"#viral #trending")
        ...
        with col2:
            ...
            if not user_videos:
                st.markdown("<div style='text-align:center;color:#8B8FA8;padding:40px 0;'><div style='font-size:40px;'>🎬</div><div style='margin-top:12px;'>No videos yet</div></div>", unsafe_allow_html=True)
            else:
                for v in user_videos[:5]:
                    vid_id, uid, title, hashtags, filename, uploaded_at, vstatus = v
                    sc = "#43D9AD" if vstatus=="published" else "#FFB547"
                    st.markdown(f"""
                    <div style='border:1px solid #2A2D45;border-radius:10px;padding:12px;margin:8px 0;'>
                        ...
                        <div style='display:flex;justify-content:space-between;margin-top:8px;'>
                            ...
                            <span style='font-size:11px;color:{sc};background:rgba(255,181,71,0.1);padding:2px 8px;border-radius:100px;'>{vstatus.upper()}</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            if user.get('plan') != 'premium':
                if st.button(f"🚀 Buy Now — {price}/mo", key="buy_premium", use_container_width=True):
                    ...
                    <div style='background:rgba(108,99,255,0.1);border:1px solid rgba(108,99,255,0.3);border-radius:12px;padding:16px;margin-top:8px;'>
                        ...
                        <a href='https://wa.me/{wa}?text=Hi! I want to upgrade to Premium plan on SocioSaas Pro. Email: {user.get("email")}' target='_blank' class='wa-button' style='display:inline-block;margin-top:10px;'>
                            📲 WhatsApp Us
                        </a>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    # ── Contact Admin (WhatsApp Manager) ─────────────────────────────────────
    with tab4:
        ...
        with col1:
            ...
            msg_text = st.text_area("Your Message", height=120, placeholder="Describe your issue or question...", key="contact_msg")
        ...
        with col2:
            ...
            <div style='text-align:center;padding:16px 0;'>
                ...
                <a href='https://wa.me/{wa}?text=Hi! I need help with SocioSaas Pro. Account: {user.get("email","")}' target='_blank' class='wa-button' style='display:inline-block;margin-top:12px;padding:14px 28px;'>
                    Open WhatsApp Chat
                </a>
    ...
    with tab5:
        ...
        with col2:
            ...
            st.markdown(f"""
            <a href='https://wa.me/{wa}?text=Hi! Please reset my device ID. My account email: {user.get("email","")}' target='_blank' class='wa-button'>

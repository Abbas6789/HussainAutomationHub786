"""
shared/uploaders.py
Platform upload functions. Currently structured as production-ready stubs.
Swap in real API calls once you have valid credentials.
"""
import time, os
from pathlib import Path


def upload_youtube(video_path: Path, title: str, description: str, hashtags: list, api_key: str) -> dict:
    ...
    try:
        ...
        # return {"ok": True, "platform": "YouTube", "video_id": response['id']}
        # ────────────────────────────────────────────────────────────────────
        time.sleep(1.2)  # simulate
        return {"ok": True, "platform": "YouTube", "video_id": f"yt_{int(time.time())}",
                "url": "https://youtube.com/shorts/demo"}
    except Exception as e:
        return {"ok": False, "platform": "YouTube", "error": str(e)}


def upload_facebook(video_path: Path, title: str, description: str, hashtags: list, token: str) -> dict:
    ...
    try:
        ...
        # r = requests.post(
        #     "https://graph.facebook.com/v19.0/me/video_reels",
        ...
        # )
        # return {"ok": True, "platform": "Facebook", "video_id": video_id}
        # ────────────────────────────────────────────────────────────────────
        time.sleep(1.2)
        return {"ok": True, "platform": "Facebook", "video_id": f"fb_{int(time.time())}",
                "url": "https://facebook.com/reels/demo"}
    except Exception as e:
        return {"ok": False, "platform": "Facebook", "error": str(e)}


def upload_tiktok(video_path: Path, title: str, description: str, hashtags: list, api_key: str) -> dict:
    ...
    try:
        ...
        # # Step 2: Upload file
        # with open(video_path, 'rb') as vf:
        #     requests.put(upload_url, data=vf,
        ...
        # ────────────────────────────────────────────────────────────────────
        time.sleep(1.2)
        return {"ok": True, "platform": "TikTok", "publish_id": f"tt_{int(time.time())}",
...
def process_video_ffmpeg(src: Path, out_dir: Path, platform: str) -> Path:
    """Convert video to 9:16 1080×1920 for the target platform."""
    ...
    try:
        ...
        subprocess.run([
            "ffmpeg", "-y", "-i", str(src),
            "-vf", filt, "-c:v", "libx264", "-preset", "fast", "-crf", "23",
